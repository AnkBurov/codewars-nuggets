...
data class BoardModel(val size: Int,
    ...
    fun getAdjacentCells(row: Int, column: Int): List<Cell> {
        val adjacentCells = mutableListOf<Cell>()
        for (i in -1..1) {
            for (j in -1..1) {
                if (i == 0 && j == 0) continue // exclude center

                val checkingRow = row + i
                val checkingColumn = column + j

                if (isOnBoard(checkingRow, checkingColumn)) {
                    adjacentCells.add(checkingRow to checkingColumn)
                }
            }
        }
    ...
    private fun isAnyAdjacentCellFilled(row: Int, column: Int): Boolean {
        for (i in -1..1) {
            for (j in -1..1) {
                ...
                if (isOnBoard(checkingRow, checkingColumn) && rows[checkingRow][checkingColumn] != null) {
                    return true
                }
            }
        }
    ...
    private fun isOnBoard(checkingRow: Int, checkingColumn: Int) = (checkingRow >= 0 && checkingColumn >= 0
